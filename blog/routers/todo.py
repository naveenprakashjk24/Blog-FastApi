...
def updateTodo(id:int,request : schemas.Todo, db: Session = Depends(database.get_db), current_user:schemas.ShowUser = Depends(oauth2.get_current_user)):
    todo = db.query(models.Todo).filter(models.Todo.id==id)
    if not todo.first():
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f'Blog with {id} not found')
    todo.update({models.Todo.title:request.title, models.Todo.description:request.description, models.Todo.priority:request.priority})
    db.commit()
